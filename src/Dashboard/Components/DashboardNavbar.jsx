  ...
  return (
    <>
      ...
      <div className={`bg-[#00008B] text-white h-16 md:flex md:justify-around items-center mx-auto ${isSticky ? 'sticky top-0 z-10' : ''} ${isMobile ? 'hidden' : 'block'}`}>
        ...
        <div className='flex'>
          <Header title="Dashboard"
            ...
            li1="Add New Services" li2="Service List" li3='Add TeamMembers' path="/Adashboard" path1="/AaddServices" path2="/AservicesList" path3='/Aaddteam' />

          <Header title="Services" icon={<MdMiscellaneousServices size={20} />} path="/Aservices" />
          <Header title="User" icon={<FaUserTie size={20} />} li1="LogIn" li2="Register" li3="Forget Password" path="/Auser" path1="/Alogin" path2="/Asignup" path3="/Aforget-pass" />
        </div>
      ...
      h-auto px-4 ${isNavOpen ? 'block' : 'hidden'} transition-all duration-1000 ease-in-out`} data-aos="fade-right"  data-aos-easing="linear" >
        <Link to="/Adashboard" className='block py-2 '>Dashboard</Link>
        <Link to="/Aservices" className='block py-2 '>Services</Link>
        <Link to="/AservicesList" className='block py-2 '>Services List</Link>

        <Link to="/Auser" className='block py-2'>User</Link>
        {isNavOpen && (
          <div>
            <Link to="/Alogin" className='block py-2'>Login</Link>
            <Link to="/Asignup" className='block py-2'>Register</Link>
          </div>
        )}
      </div>
    </>
